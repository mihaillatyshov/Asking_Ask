...
def index(request):
    context = Context({
        'questions': models.TEST_QUESTIONS,
        'page_obj': paginate(models.TEST_QUESTIONS, request)})
    return render(request=request, template_name='index.html', context=context)


def question(request, id: int):
    context = Context({
        'question': models.TEST_QUESTIONS[id],
        'answers': models.TEST_ANSWERS})
